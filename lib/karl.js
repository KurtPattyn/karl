...
var colors = {
  ...
  _colorize: function(msg, color) {
    return '\u001b[' + this._colors[color][0] + 'm' + msg + '\u001b[' + this._colors[color][1] + 'm';
  },
...
function textFormatter(message) {
  if (logOptions.includeLocationInformation) {
    return util.format("[%s] %s - %s - %s[%s@%s(%d)]: %s", message.level, message.timestamp, message.hostName, message.process.name,
                                                      message.functionName, message.fileName, message.lineNumber,
                                                      message.message);
  } else {
    return util.format("[%s] %s - %s - %s: %s", message.level, message.timestamp, message.hostName, message.process.name, message.message);
  }
}
...
var formatters = {
  ...
  getFormatter: function(options) {
    return !options.humanReadable ? formatters.jsonFormatter : (options.json ? formatters.jsonPrettyPrintFormatter : formatters.textFormatter);
  }
}
...
function getStackInfo(limFn) {
  ...
  Error.captureStackTrace(this, limFn || getCaller3Info);
  ...
  karl[level.toLowerCase()] = function _log() {
    ...
    setImmediate(function() {
      ...
      var message = {
        ...
        fileName: path.basename(caller.getFileName()),
